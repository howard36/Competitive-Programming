...
int finddep(int node)
{
	if (dependent[node].x == node)
		return node;
	int realdep = finddep(dependent[node].x);
	dependent[node].y ^= dependent[dependent[node].x].y ^ 1;
	dependent[node].x = realdep;

	return dependent[node].x;
}
